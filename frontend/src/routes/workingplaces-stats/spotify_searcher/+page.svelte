<script>
    ...
    async function callApi(method, url, body) {
        ...
        if(status == 200){
            ...
            playlists = [];
            console.log(data)
            data.tracks.items.forEach(item => addPlaylist(item)); 
    ...
    async function addPlaylist(item){
        ...
        playlists.push({
            ...
            url_image: item.album.images[1].url
        });
    }
    ...
    async function refreshPlaylists(){
        ...
        callApi( "GET", `https://api.spotify.com/v1/search?q=remaster%2520track%3A${busqueda}&type=track&offset=0`, null);
    }
            ...
            <div class="row">
                ...
                <Form>
                    ...
                    <Table  bordered striped>
                        <thead>
                            <tr>
                                <th><h3 style="margin-right: 1%;">Canciones</h3> <Button style="margin-right: 1%;" color="warning" on:click={refreshPlaylists}>Recargar</Button> <Button color="danger" on:click={revokeAcces}>Limpiar credenciales</Button></th>
                            </tr>
                        </thead>
                        <tbody>
